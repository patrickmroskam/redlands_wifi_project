...
| Database already contains a duplicate or malformed BSSID | **The run stops (exit 2)**. Nothing is written or deleted, and the report lists the offending records by index. The logs wait in `ingest/` until a human fixes `data/networks.json` in a PR. A unit test (`CommittedDatabase`) catches this in CI when that PR is opened. |
