...
| Same BSSID in a *different* database | Not a duplicate. Dedupe is per-database: a dual-radio device answering on one address belongs in both `networks.json` and `bluetooth.json`, and a Flock camera is published to `flock.json` *as well as* the map its `Type` selects. The `in database` / `in this batch` split reads the same way — only the databases a row is routed to, as they were before the run, count towards `in database` (#41). |
...
| Row whose `Type` is neither `WIFI` nor `BLE` | Never published; counted as **not wifi**. Cell rows (GSM/LTE) carry tower IDs in the `MAC` column, so they register nothing. A classic-Bluetooth row (`Type=BT`) carries a real MAC and the device's name, so an opt-out suffix (or an unreadable name) on it still withholds that address from every database for the whole batch (#50). |
